...
class CrudViewGenerator extends BaseCommand
{
    ...
    protected function parseTemplate(string $class, array $search = [], array $replace = [], array $data = []): string
    {
        ...
        $lengthArray = count($controllerName);
        unset($controllerName[$lengthArray-1]);
        unset($controllerName[$lengthArray-2]);
        unset($controllerName[$lengthArray-3]);
        $model     = implode('\\', $controllerName).'\\Models\\'.pascalize($this->params['directory']);
        $search[]  = '<@x-';
        $search[]  = '{table}';
        $search[]  = '{model}';
        $search[]  = '{form-content}';
    ...
    private function generateFormInput($field, $objForeignKey)
    {
        ...
        $mapType    = $this->mapTypeData[$field->type] ?? $field->type;
        $required   = 'required';
        ...
        switch($mapType){
            case 'numeric':
                $validation = "x-validate.number x-mask:dynamic=\\\"\\\$money(\\\$input, ',', '.', 2)\\\"";
                break;
            case 'date':
                $validation = "x-validate.date x-mask='9999-99-99' placeholder='YYYY-MM-DD'";
                break;
            default:
        }
        switch ($type) {
            case 'text':
                $formInput = <<<FIELD
                    <?= form_textarea('{$name}', old('{$name}', \$data->{$name} ?? ''), "rows='4' {$validation} class='{$classInput}' {$required} placeholder='".lang('crud.{$this->tableName}.{$name}')."' ") ?>
                    FIELD;
                break;
            ...
            case 'select2':
                ...
                $formInput   = <<<FIELD
                    <?= form_dropdown('{$name}',\${$optionItems} ,old('{$name}', \$data->{$name} ?? ''), "class='{$classInput}' {$required} {$validation} placeholder='".lang('crud.{$this->tableName}.{$name}')."' ") ?>
                    FIELD;
                break;

            default:
                $formInput = <<<FIELD
                    <?= form_input('{$name}', old('{$name}', \$data->{$name} ?? ''), "class='{$classInput}' {$required} {$validation} placeholder='".lang('crud.{$this->tableName}.{$name}')."' ") ?>
                    FIELD;
        }
