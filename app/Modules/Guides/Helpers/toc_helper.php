...
if (! function_exists('load_recursive_guide')) {
    ...
    {
        // Name of folder
        $string = '<h3>' . esc(ucwords(trim(App\Modules\Guides\Libraries\GuideCollection::formatPage($folder), ' /'))) . '</h3>
            ...
            if (is_numeric($sub_folder)) {
                ...
                $string .= '<li>
								<a href="/' . ADMIN_AREA . '/guides/' . $alias . '-' . trim($folder, ' /') . '/' . trim($row, ' /') . '">
									' . esc(App\Modules\Guides\Libraries\GuideCollection::formatPage($row)) . '
								</a>
							</li>';
            } else {
                // Is subfolder
